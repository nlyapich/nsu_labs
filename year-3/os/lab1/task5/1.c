...
{
  sigset_t signal_set; // маска
  sigfillset(&signal_set); // ставит единицы напротив всех битов в маске
  pthread_sigmask(SIG_BLOCK, &signal_set, NULL); // sigprocmask()
...
void *sigint_handling_routine(void *arg)
{
  ...
  sigset_t set;
  sigemptyset(&set); // ставит нули напротив всех битов в маске
  sigaddset(&set, SIGINT); // ставит единицу напротив соответствующего бита сигнала
  sa.sa_mask = set;
